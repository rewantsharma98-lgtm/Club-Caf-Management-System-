...
export default function QrMenuPage({ initialCode }: QrMenuPageProps) {
  ...
  useEffect(() => {
    ...
    if (segments.length >= 3 && segments[0] === 'menu' && segments[1] === 'qr') {
      setCode(segments.slice(2).join('/'));
      return;
    }
    ...
    <main className="mx-auto min-h-screen max-w-lg bg-ink px-5 py-8 pb-[max(2rem,env(safe-area-inset-bottom))] pt-[max(2rem,env(safe-area-inset-top))]">
      ...
      {!loading && !code && (
        <div className="text-center text-body">
          <p>No QR code was provided.</p>
          <p className="mt-3 text-sm text-slate">
            Use <span className="font-medium">/menu/qr?code=YOUR_CODE</span> or visit a QR link like{' '}
            <span className="font-medium">/menu/qr/ABC123</span>.
          </p>
        </div>
      )}
          ...
          {data.items.map((item) => (
            <article key={item._id} className="flex gap-4 border-b border-border pb-4">
              ...
              <div className="flex-1">
                ...
                {item.description && <p className="text-xs text-body mt-1">{item.description}</p>}
              </div>
            </article>
          ))}
        </div>
